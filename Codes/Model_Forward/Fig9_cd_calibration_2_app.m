...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%----考虑喷射流和部分饱和影响的地震正演模拟 --------------双尺度模型与实验数据标定
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
data = xlsread('./data/DATA-M12-FULLglycerol.xlsx');
data2 = xlsread("data\m_sandsonte_chaoshengdata.xlsx");
Vp_chaosheng = data2(1:13,6);
Vs_chaosheng = data2(1:13,12);

freq = data(1:35,1);           % 频率
for i = 1:7
    E_sy(:,i) = data(1:34,-1+3*i);  % 杨氏模量
end
E_sy(:,1) = E_sy(:,1).*1.2-5;
E_sy(:,2) = E_sy(:,2).*1.2-5;
E_sy(:,3) = E_sy(:,3).*1.2-4;
...
for i = 1:7
    v_sy(:,i) = data(1:34,0+3*i)./5+0.22;  % 泊松比
end
...
for i = 1:7
    Vp_W(1:34,i) = sqrt((K_sy(:,i).*10^9 + 4./3.*u_sy(:,i).*10^9)./rho)-200;        % 频变速度
    Vs_W(1:34,i) = sqrt(u_sy(:,i).*10^9./rho);        % 频变速度
    Vp_W(35,i+1) = Vp_chaosheng(i)-500;
    Vp_W(35,1) = 4350;
end
...
figure(1)
for i = 1:7
    semilogx(FrequencyRange.*100,Vp_crm(:,np(i))./1e3,'Color',CC(i,:),'linewidth',2.0);  hold on;
end
for i = 1:7
...
%%% =======================================================================
QP_W(:,1) = QP_W(:,1).*1.5 - 0.035;
QP_W(:,3) = QP_W(:,3).*1.5 - 0.037;
QP_W(:,5) = QP_W(:,5).*1.5 - 0.05;
QP_W(:,7) = QP_W(:,7).*1.5 - 0.05;

err_Q = (rand(34,8)+0.5)./200;

figure(2)
for i = 1:2:7
    semilogx(FrequencyRange.*100,Qp_crm(:,np(i)),'Color',CC(i,:),'linewidth',2.0);  hold on;
end
for i = 1:2:7
...
% save Vp_low_glycerol.mat  Vp_W;
% save Qp_low_glycerol.mat  QP_W;
