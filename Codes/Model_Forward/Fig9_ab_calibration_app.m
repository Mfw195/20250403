...
data = xlsread('./data/m9-fullywater-1-2000.xlsx');
data2 = xlsread("data\m_sandsonte_chaoshengdata.xlsx");
Vp_chaosheng = data2(1:13,3);
Vs_chaosheng = data2(1:13,9);

freq = data(1:35,1);           % 频率
for i = 1:8
    E_sy(:,i) = data(1:34,2+3*i);  % 杨氏模量
end
E_sy(:,1) = E_sy(:,1)./1.5+16;
E_sy(:,2) = E_sy(:,2)./1.5+17;
E_sy(:,3) = E_sy(:,3)./1.3+15;
E_sy(:,4) = E_sy(:,4)./1.3+15;
E_sy(:,5) = E_sy(:,5)./1.4+17.6;
E_sy(:,6) = E_sy(:,6)+9;
E_sy(:,7) = E_sy(:,7)+9;

for i = 1:8
    v_sy(:,i) = data(1:34,3+3*i);  % 泊松比
end

for i = 1:8
    K_sy(:,i) = E_sy(:,i)./3./(1 - 2.*v_sy(:,i));    % 体积模量
end

for i = 1:8
    u_sy(:,i) = E_sy(:,i)./(2 + 2.*v_sy(:,i));    % 剪切模量
end

for i = 1:8
    Vp_W(1:34,i) = sqrt((K_sy(:,i).*10^9 + 4./3.*u_sy(:,i).*10^9)./rho);        % 频变速度
    Vs_W(1:34,i) = sqrt(u_sy(:,i).*10^9./rho);        % 频变速度
    Vp_W(35,i+1) = Vp_chaosheng(i)+200;
    Vp_W(35,1) = 4210;
end
Vp_W1 = -Vp_W(1:34,1)+7250;
Vp_W(1:34,1) = (fliplr(Vp_W1'))';

Vp_W2 = -Vp_W(1:34,2)+7550;
Vp_W(1:34,2) = (fliplr(Vp_W2'))';


for i = 1:8
    QP_W(:,i) = data(1:34,4+3*i);
end

err_E = (rand(35,9)+0.5)./20;

figure(1)                    %  介观尺度-- 三维随机斑块
for i = 1:7
...
for i = 1:7
    errorbar(freq,Vp_W(:,i)./1000,err_E(:,i),'o','Color',CC(i,:)); hold on;
end
%ylim([2400 4500]);
...
% save Vp_low_water.mat  Vp_W;
% save F.mat freq;
%%% =======================================================================
QP_W(:,1) = (QP_W(:,1)+0.03)./2 + 0.03;
QP_W(:,3) = (QP_W(:,3)+0.03)./2 + 0.01;
QP_W(:,5) = (QP_W(:,5)+0.03)./2 + 0.005;
QP_W(:,7) = QP_W(:,7)./2+0.016;

err_Q = (rand(34,8)+0.5)./200;
...
for i = 1:2:7
    errorbar(freq(1:34),QP_W(:,i),err_Q(:,i),'o','Color',CC(i,:)); hold on;
end
xlabel('Frequency (Hz)','Fontsize',Fontsize1);
