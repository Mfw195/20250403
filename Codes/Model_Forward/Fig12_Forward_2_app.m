clc;    clear all;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%----考虑喷射流和部分饱和影响的地震正演模拟--------------饱和度对正演结果的影响！！！！！！！-------------AVO效应
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%============================================
...
Vs_1 = [Vs1 Vs2 Vs_rock(1)./1000 Vs4];             %  每层的横波速度(km/s)
Vs_2 = [Vs1 Vs2 Vs_rock(2)./1000 Vs4];             %  每层的横波速度(km/s)
Vs_3 = [Vs1 Vs2 Vs_rock(3)./1000 Vs4];             %  每层的横波速度(km/s)
Vs_4 = [Vs1 Vs2 Vs_rock(4)./1000 Vs4];             %  每层的横波速度(km/s)
Vs_5 = [Vs1 Vs2 Vs_rock(5)./1000 Vs4];             %  每层的横波速度(km/s)

rho_s = 2650;
por = 0.12;   
S1 = [0.1 0.3 0.5 0.7 0.9];   %  S_water : 流体1的饱和度
rho_f = [rho_water rho_gas];            % 各流体的密度
rho = rho_s.*(1-por) + por.*S1.*rho_f(1) + por.*(1 - S1).*rho_f(2);
rho_1 = [rho1 rho2 rho(1)./1000 rho4];          %  每层的密度(g/cm^3)
rho_2 = [rho1 rho2 rho(2)./1000 rho4];          %  每层的密度(g/cm^3)
rho_3 = [rho1 rho2 rho(3)./1000 rho4];          %  每层的密度(g/cm^3)
rho_4 = [rho1 rho2 rho(4)./1000 rho4];          %  每层的密度(g/cm^3)
rho_5 = [rho1 rho2 rho(5)./1000 rho4];          %  每层的密度(g/cm^3)
...
figure;hold on;
wigb(syna_p_r_1,scale,ang_inc,tt1);    % 反射率法
ylim([0 350]);
...
figure;hold on;
wigb(syna_p_r_3,scale,ang_inc,tt1);    % 反射率法
ylim([0 350]);
xlabel('Incident angle (Degree)');
ylabel('Time');
...
figure;hold on;
wigb(syna_p_r_5,scale,ang_inc,tt1);    % 反射率法
ylim([0 350]);
xlabel('Incident angle (Degree)');
ylabel('Time');
