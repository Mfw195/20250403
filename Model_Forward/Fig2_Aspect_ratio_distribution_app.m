clc;    clear all;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% =====     The pore aspect ratio distribution of the core was inversely calculated based on the ultrasonic data    ========================
Fontsize = 14;              % For display
AspectRatio = 0.618; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rho_frame = 2820;                                  % Density of solid grains: kg/m^3
np=7;  CC=colormap(jet(np));
%%%%%%% The variation of rock skeleton velocity with pressure ============================================
...
%%% espilon, The cumulative fracture density of all unclosed soft pores in the rock
...
z1=log(espilon1(1:end-1));       %%% Fitting the curves  
...
% K_d & um_d: the dry effective modulus; 
...
po_d = 0.5*lamda_d./(lamda_d+um_d);        

lamda_st = Kdry_c(end)-2*um_dry_c(end)/3;                   % High-freq limit；
...
alpha_p=zeros(length(pp),length(pp));     

iei=[0 ie(1:end-1)-ie(2:end)];       % The fracture densities corresponding to each aspect ratio (or different pressure)
iek=[0 ie1(1:end-1)-ie1(2:end)];     
ieu=[0 ie2(1:end-1)-ie2(2:end)];     
...
xlabel('Aspect ratio (\alpha)','fontsize',Fontsize);      
ylabel('Crack porosity (%) ','fontsize',Fontsize);       
...
for np=1:length(pp)
    for ix=mn(np):length(pp)
        ...
        ieu1(ix,np)=ieu1(ix-1,np)+ieu1(ix,np);     %  Cumulative fracture density
    end
end
...
xlabel('Aspect ratio (\alpha/10^3)','fontsize',Fontsize);      
ylabel('Cumulative crack density');            
